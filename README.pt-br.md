...
Protopanda é uma plataforma open source (firmware e hardware) para controlar protogens. A ideia é ser simples o suficiente para que você só precise de um pouco de conhecimento técnico para fazê-lo funcionar, mas ao mesmo tempo flexível o bastante para que uma pessoa com o mínimo de conhecimento de Lua possa fazer quase tudo.
...
- RGB de 16 bits
- Suporte para LED endereçável WS2812
- Customização usando Lua
- Expressões faciais são só arquivos `.PNG`
- Utiliza um cartão SD com configurações fáceis de ajustar
- Suporte BLE para controle remoto, ou infravermelho
- Alimentado via USB-C
- Tela interna com menuzinho
- Modo Wi-Fi, com editorzinho
...
__TL;DR: Use um power bank de pelo menos 20W com PD e USB-C.__
...
Cada painel HUB75 pode consumir até 2A no brilho máximo, então alimentar diretamente via USB em 5V pode ser problemático. Por isso, esta versão com o regulador ativa o PD (Power Delivery) no USB, solicitando 9V a 3A, o que fornece energia suficiente para ambos os painéis. Infelizmente, esta versão consome muito mais energia.

Na maioria dos casos, você não estará operando os painéis no brilho máximo nem com todos os LEDs em branco, então a versão em 5V é a recomendada. O Protopanda também suporta [fitas de LED](#fitas-de-led), e há uma porta dedicada a elas. A saída também é de 5V, a mesma dos painéis. Como os LEDs são do tipo WS2812B, eles podem consumir até 20 mA por LED a 100% de brilho.
...
- [Sobreposições (Overlays)](#sobreposições-overlays)
...
> Modificar o arquivo `animation.json` adicionando ou removendo arquivos força o sistema a reconstruir o [arquivo bulk de frames](#arquivo-bulk).
...
* **pattern** (string)
  ...
  ```
  Isso carregará de `/bolinha/input-onlinegiftools-1.png` até `/bolinha/input-onlinegiftools-155.png`.

* **flip_left** (bool)
  Devido à orientação dos painéis, pode ser necessário inverter o lado esquerdo horizontalmente.

* **flip_right** (bool)
  Devido à orientação dos painéis, pode ser necessário inverter o lado direito horizontalmente.

* **name** (string)
  As animações funcionam basicamente assim:
  ```
  Desenhar frame 1
  esperar um tempo
  Desenhar frame 2
...
Uma vez que os frames são carregados e a execução começa, é trabalho dos [scripts Lua](#programação-em-lua) gerenciar as expressões.
...
- **`animation`** (int[] ou `"auto"`)
  - `int[]`: O ID de cada frame a ser exibido (ex.: `[1, 2, 3]`).
  - `"loop"`: Os frames são adicionados automaticamente em sequência (ex.: `1, 2, 3...`).
  - `"pingpong"`: Os frames são adicionados automaticamente em sequência e depois repetidos invertidos (ex.: `1, 2, 3... ...3, 2, 1`).
  - `"loop_backwards"`: Mesmo comportamento do loop, porém de trás pra frente (ex.: `...3, 2, 1`).

- **`duration`** (int)
  A duração de cada frame.

- **`hidden`** (string)
  Esconde a expressão da seleção no menu.

- **`intro`** (string)
  Esse parâmetro deve ser o nome de outra animação, que DEVE ser uma `transition=true`. Essa animação de introdução toca sempre que essa expressão entra.

- **`outro`** (string)
  Esse parâmetro deve ser o nome de outra animação, que DEVE ser uma `transition=true`. Essa animação de encerramento toca sempre que essa expressão sai de cena.

- **`transition`** (boolean)
  Transforma no tipo `transition`. Isso faz com que essa animação toque apenas uma vez e, quando chamada, seja inserida na pilha de animações sem substituir a atual — apenas tocar uma vez e voltar para a anterior.

- **`repeats`** (int, padrão 1)
  Se a expressão é do tipo `transition`, você pode fazer com que ela se repita N vezes.

- **`overlay`** (string)
  Nome da sobreposição (overlay) a ser usada nessa animação.

- **`onEnter`** (string, código Lua)
  Quando a animação assume o controle da tela, executa um código Lua.

- **`onLeave`** (string, código Lua)
...
Às vezes você quer algo com um pouco mais de estilo, como uma boca que se move enquanto você fala, algumas estrelas, ou até mesmo algo que reaja a um acelerômetro. Para isso, você pode criar sobreposições:
...
As expressões são armazenadas em uma pilha. Quando você adiciona uma animação que não se repete, ela pausa a animação atual e executa até o final da nova animação. Se você adicionar duas ao mesmo tempo, a última será executada; quando terminar, a anterior volta a executar.

### Arquivo Bulk

Mesmo com o cartão SD, mudar os quadros não é tão rápido — a interface do cartão SD não é rápida o suficiente. Para acelerar, as imagens são decodificadas de PNG para dados brutos de pixels no formato RGB565 e armazenadas na flash interna. Todos os quadros são armazenados em um único arquivo chamado `arquivo bulk`. Isso é feito de forma que os quadros fiquem armazenados sequencialmente e, mantendo o arquivo aberto, a velocidade de transferência é acelerada, atingindo 60 FPS.

Toda vez que você adiciona ou modifica um quadro, é necessário reconstruir esse arquivo. Isso pode ser feito no menu ou chamando a função Lua `composeBulkFile`.

### Modo Managed

As animações são processadas pelo Núcleo 0, então você não precisa perder tempo precioso atualizando-as nos [scripts Lua](#programação-em-lua). É possível mudar o quadro usando scripts Lua, mas isso também é um desperdício — então deixe isso para o outro núcleo, e você só precisa se preocupar em selecionar quais expressões deseja!

Durante o modo managed, o desenho dos quadros é tratado pelo Núcleo 0.

![Modo Managed](doc/managed.png "Modo Managed")
...
O Protopanda suporta o protocolo de LED endereçável WS2812B e fornece um sistema simples e direto para definir alguns comportamentos para a fita/matriz.

![Fita de LED](doc/A7301542.JPG)

![Diagrama da fita de LED](doc/ewm.drawio.png)
...
Todas as keybinds acima mapeiam para o input do controle do Protopanda.

## Hardware

O Protopanda foi projetado para rodar no ESP32-S3-N16R8, uma versão com 16MB de flash, 384KB de ROM, 512KB de RAM e 8MB de PSRAM octal. Essa versão com mais espaço e PSRAM é necessária para ter RAM suficiente para rodar os painéis, BLE e Lua simultaneamente.

No hardware, há uma porta para os dados HUB75, um conector para cartão SD, dois terminais parafusados para a saída de 5V, os pinos de alimentação, uma porta I2C e um pino para fita de LED.
...
![Esquemático](doc/schematic.png "Esquemático")
...
* **Núcleo 0**
  Por padrão, o Núcleo 0 é projetado principalmente para gerenciar o Bluetooth. Quando não está fazendo isso, ele gerencia as animações e, quando o [Modo Managed](#modo-managed) está ativo, também cuida da atualização da tela de LED.
...
__[Referência de funções Lua](doc/lua-doc.pt-br.md)__
